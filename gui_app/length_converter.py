...
def main():
    ...
    root.geometry("400x400")    # set screen size

    t1 = ttk.Entry(root, width=20, font=('Arial', 20))  # text box
    units = ['meters', 'centimeters', 'feet', 'inches']
    selected_unit = tk.StringVar()
    drop_down = ttk.Combobox(root, textvariable=selected_unit, values=units, font=('Arial', 20))
    drop_down.set(units[0]) # first(top) text on drop-down box
    l1 = ttk.Label(root, text='', font=('Arial', 20))
    b1 = tk.Button(root, text='Convert', font=('Arial', 20), 
                   command= lambda:convert(t1, selected_unit, l1))
